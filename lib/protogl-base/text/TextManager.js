var TextManager = function(game) {
	...
	this.addString = function(charSequence, size, pos, col) {
		this.strings.push({text: charSequence, fontSize: size, x: pos.x, y: pos.y, r: col.x, g: col.y, b: col.z});
	};
	...
	this.getVerts = function() {
		...
		for (var i = 0; i < this.transformedStrings.length; i++) {
			...
			for (var j = 0; j < this.strings[i].text.length; j++) {
				tempVerts.push([
					startPoint.x + (this.strings[i].fontSize * j), startPoint.y, this.transformedStrings[i][j].u1, this.transformedStrings[i][j].v1, this.strings[i].r, this.strings[i].g, this.strings[i].b, 0, 1, 0, 0,
					startPoint.x + (this.strings[i].fontSize * j) + this.strings[i].fontSize, startPoint.y, this.transformedStrings[i][j].u2, this.transformedStrings[i][j].v1, this.strings[i].r, this.strings[i].g, this.strings[i].b, 0, 1, 0, 0,
					startPoint.x + (this.strings[i].fontSize * j) + this.strings[i].fontSize, startPoint.y + this.strings[i].fontSize, this.transformedStrings[i][j].u2, this.transformedStrings[i][j].v2, this.strings[i].r, this.strings[i].g, this.strings[i].b, 0, 1, 0, 0,

					startPoint.x + (this.strings[i].fontSize * j) + this.strings[i].fontSize, startPoint.y + this.strings[i].fontSize, this.transformedStrings[i][j].u2, this.transformedStrings[i][j].v2, this.strings[i].r, this.strings[i].g, this.strings[i].b, 0, 1, 0, 0,
					startPoint.x + (this.strings[i].fontSize * j), startPoint.y + this.strings[i].fontSize, this.transformedStrings[i][j].u1, this.transformedStrings[i][j].v2, this.strings[i].r, this.strings[i].g, this.strings[i].b, 0, 1, 0, 0,
					startPoint.x + (this.strings[i].fontSize * j), startPoint.y, this.transformedStrings[i][j].u1, this.transformedStrings[i][j].v1, this.strings[i].r, this.strings[i].g, this.strings[i].b, 0, 1, 0, 0
						        ]);
			}
